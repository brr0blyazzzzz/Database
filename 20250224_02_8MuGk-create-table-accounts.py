...
steps = [
  ...
  CONSTRAINT family CHECK (family IS NOT NULL AND TRIM(BOTH FROM family) <> '' AND family ~ '^[A-Za-z].*[A-Za-z]$'),
  CONSTRAINT name CHECK (name IS NOT NULL AND TRIM(BOTH FROM name) <> '' AND name ~ '^[A-Za-z].*[A-Za-z]$'),
  CONSTRAINT patronymic CHECK (patronymic IS NULL OR TRIM(BOTH FROM patronymic) = '' OR (patronymic ~ '^[A-Za-z].*[A-Za-z]$')),
